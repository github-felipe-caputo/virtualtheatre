...
public class KinectBodyRecorded : MonoBehaviour {
    ...
    void Update()
    {
        ...
        else if (ParticleMan || StarMan) // Their bodies are similar, the refresh can be the same
        {
    ...
    private void sampleKinectData(string data, float timeRate, int frameRate)
    {
        ...
        for (int i = 0; i < lines.Length - 1; i += frameRate)
        {
            ...
            float timeOfCurrentFrame = float.Parse(values[0]);

            // If it's on the range we are interested in
            if (timeOfCurrentFrame > timeStart && timeOfCurrentFrame < timeEnd)
            {
                // Kinect = 25 joints
                for (int joint = 0; joint < 25; ++joint)
                {
                    // Kinect doesn't need rotations, so for that we use 0 0 0
                    str[joint] += currentTime + " " + values[joint * 3 + 1] + " " + values[joint * 3 + 2] + " " + values[joint * 3 + 3] + " 0.0 0.0 0.0" + Environment.NewLine;
                }
    ...
    public void OnPlayClicked()
    {
        if (playFlag == false)
        {
            GameObject.Find("PlayText").GetComponent<Text>().text = "Pause";
